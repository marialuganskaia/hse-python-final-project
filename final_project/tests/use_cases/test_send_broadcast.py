...
from final_project.src.hackathon_assistant.domain.models import User, ReminderSubscription, UserRole
...
class TestSendBroadcastUseCase:
    ...
    @pytest.mark.asyncio
    async def test_get_broadcast_targets(self, use_case_send_broadcast, mock_user_repo, mock_subscription_repo):
        ...
        users = [
            User(id=1, telegram_id=111, username="user1", first_name="Alice", role=UserRole.PARTICIPANT),
            User(id=2, telegram_id=222, username="user2", first_name="Bob", role=UserRole.PARTICIPANT),
            User(id=3, telegram_id=333, username="user3", first_name="Charlie", role=UserRole.PARTICIPANT),
            User(id=4, telegram_id=444, username="user4", first_name="Diana", role=UserRole.PARTICIPANT),
            User(id=5, telegram_id=555, username="user5", first_name="Eve", role=UserRole.PARTICIPANT),
        ]
        mock_user_repo.get_by_hackathon.return_value = users

        result = await use_case_send_broadcast.execute(hackathon_id=hackathon_id, message="Test broadcast")
    ...
    @pytest.mark.asyncio
    async def test_get_broadcast_targets_no_subscriptions(self, use_case_send_broadcast, mock_user_repo, mock_subscription_repo):
    ...
    @pytest.mark.asyncio
    async def test_get_broadcast_targets_only_disabled_subscriptions(self, use_case_send_broadcast, mock_user_repo,
    ...
    @pytest.mark.asyncio
    async def test_get_broadcast_targets_user_not_found_for_subscription(self, use_case_send_broadcast, mock_user_repo,
                                                                         mock_subscription_repo):
        ...
        subscriptions = [
            ReminderSubscription(id=1, user_id=1, hackathon_id=5, enabled=True),
            ReminderSubscription(id=2, user_id=999, hackathon_id=5, enabled=True),  # несуществующий user_id
        ]
        mock_subscription_repo.get_by_hackathon.return_value = subscriptions
    ...
    @pytest.mark.asyncio
    async def test_get_broadcast_targets_empty_users(self, use_case_send_broadcast, mock_user_repo, mock_subscription_repo):
