...
class TestBotHelpers:
    ...
    async def test_is_organizer_by_allowed_ids(self, mock_use_cases):
        ...
        with patch("hackathon_assistant.adapters.bot.helpers.get_settings") as mock_settings:
            settings = MagicMock()
    ...
    async def test_is_organizer_by_role(self, mock_use_cases):
        ...
        with patch("hackathon_assistant.adapters.bot.helpers.get_settings") as mock_settings:
            ...
            assert result is True
            mock_use_cases.start_user.user_repo.get_by_telegram_id.assert_called_once_with(
                telegram_id
            )
