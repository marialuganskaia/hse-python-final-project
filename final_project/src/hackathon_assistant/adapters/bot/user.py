...
async def cmd_start(message: types.Message, use_cases: UseCaseProvider) -> None:
    ...
    await message.answer(f"Привет, {message.from_user.first_name}! Ты зарегистрирован в системе бота.")
...
async def cmd_schedule(message: types.Message, use_cases: UseCaseProvider) -> None:
    ...
    for item in schedule_items:
        ...
        schedule_text += f"  🕐 {item.starts_at.strftime('%H:%M')} - {item.ends_at.strftime('%H:%M')}\n"
        if item.location:
...
async def cmd_notify_on(message: types.Message, use_cases: UseCaseProvider) -> None:
    ...
    if success:
        ...
        await message.answer("❌ Не удалось включить уведомления. Сначала выберите хакатон (/hackathon).")
...
async def cmd_notify_off(message: types.Message, use_cases: UseCaseProvider) -> None:
    ...
    if success:
        ...
        await message.answer("⚠️ Не удалось выключить уведомления. Возможно, они уже были выключены.")
