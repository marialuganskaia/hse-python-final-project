...
async def cmd_admin_broadcast(message: types.Message, use_cases: UseCaseProvider) -> None:
    ...
    hack_code = command_parts[1]
    broadcast_message = command_parts[2]
    
    await message.answer("Рассылка пока не реализована.")


@admin_router.callback_query(BroadcastStates.choosing_hackathon, F.data.startswith("broadcast_hack_"))
async def process_hackathon_choice(callback: types.CallbackQuery, state: FSMContext, use_cases: UseCaseProvider):
    try:
...
async def process_broadcast_message(message: types.Message, state: FSMContext, use_cases: UseCaseProvider):
    try:
...
async def confirm_broadcast(callback: types.CallbackQuery, state: FSMContext, use_cases: UseCaseProvider):
    try:
        data = await state.get_data()
        hackathon_id = data.get("hackathon_id")
