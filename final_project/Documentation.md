...
Суть — создание единой точки для участников и организаторов. В боте всегда под рукой актуальное расписание, ответы на частые вопросы и оперативные объявления. Организаторы могут делать рассылки и напоминать о дедлайнах, участники — подписываться на события и получать уведомления в нужном часовом поясе. Цель — снять хаос первых дней, ускорить коммуникацию и сделать участие в хакатоне максимально спокойным и прозрачным.
...
- как послать всем напоминания, чтобы они точно нормально дошли.
...
- хранит данные в локальной БД (PostgreSQL);
...
4. **Просмотр FAQ**
   ...
   - Бот показывает список типичных вопросов и ответов:
     - регистрация, форматы защиты и т.п.
...
2. Технический администратор переводит эти данные в конфигурационный файл, где фиксируются:
...
- `src/hackathon_assistant/use_cases/` — **Use Cases (сценарии)**:
  - координация доменных операций: `GetSchedule`, `GetRules`, `GetFAQ` и т.п.;
...
4. БД: PostgreSQL.  
5. Асинхронное подключение к БД через SQLAlchemy Async.  
6. Тесты (pytest) с покрытием ≥ 65 %.  
7. Паттерны:
...
**Дробот Мария (Архитектура и инфраструктура)**

- Структура проекта и настройки – создала пакеты, конфигурацию (.env, settings), точки входа, линтеры.
- Подключение БД – реализовала async engine, сессии, health-check (db_ping).
- DI-контейнеры – создала UseCaseProvider для внедрения зависимостей.
- Интеграция слоёв – настроила middleware для передачи use cases в хендлеры бота.
- Docker и деплой – написала Dockerfile, docker-compose, инструкции для запуска.
- Следила за границами слоёв, воспроизводимостью, финальной сборкой

**Луганская Мария (Домен + БД)**
- Доменная модель – спроектировала и реализовала классы сущностей (User, Hackathon, Event, FAQ и т.д.).
- Схема БД и ORM – создала SQLAlchemy модели и ER-диаграмму.
- Миграции – настроила Alembic, сгенерировала миграции.
- Репозитории – реализовала все репозитории (UserRepo, EventRepo и др.) согласно портам из use cases.
- Use Cases (логика) – написала всю бизнес-логику для команд участника и организатора (расписание, FAQ, напоминания, статистика, рассылка).
- Фиксы БД

**Золотовицкая Злата (Telegram-бот / aiogram)**
- Роутеры и хендлеры – создала структуру роутеров (user, admin), реализовала все обработчики команд (/start, /schedule, /rules, /faq, /notify, /admin_stats, /admin_broadcast).
- Интеграция с use cases – подключила хендлеры к UseCaseProvider, обработала данные от use cases
- Форматирование ответов – создала модуль formatters для красивого вывода расписания, FAQ, статистики.
- UX/UI бота – работала над текстами, кнопками, обработкой ошибок, удобством взаимодействия.
- Отправка напоминаний
- Тестирование E2E – проверяла работу бота с реальной БД.

**Топорова Анастасия (Тесты, CI, качество)**
- Юнит-тесты – покрыла тестами доменные модели, use cases (с мок-репозиториями).
- Интеграционные тесты – написала тесты для репозиторий (работа с реальной БД) и хендлеров бота.
- Покрытие кода – довела coverage до требуемых ≥65%, анализировала отчёты.
- CI/CD – настроила пайплайн (GitHub Actions/GitLab CI) для запуска линтеров и тестов.
- Контроль качества – отслеживала стабильность тестов, помогала находить и фиксить баги
- помощь с Use cases
- Написала тесты для репозиториев
